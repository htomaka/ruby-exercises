...
class AllPatternTest < Minitest::Test
  ...
  def test_6
    ...
    all_busy = true
    friend_status.each do |key, friend|
      all_busy = false unless friend == :busy
    end
  ...
  def test_7
    ...
    zip_codes.each do |code|
      all_five_digits = false unless Math.log10(code).to_i + 1 == 5
    end
  ...
  def test_9
    ...
    snacks.each do |snack|
      all_caps = false unless snack.upcase == snack
    end
