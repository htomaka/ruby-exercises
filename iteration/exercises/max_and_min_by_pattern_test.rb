...
class MaxAndMinByPatternTest < Minitest::Test
  ...
  def test_5
    ...
    most_digits = stats[0]
    stats.each do |number|
      if number.to_s.length > most_digits.to_s.length
        most_digits = number
      end
    end
  ...
  def test_10
    ...
    programmers.each do |lang, names|
      if names.length < programmers[fewest_programmers].length
        fewest_programmers = lang.to_sym
      end
    end
