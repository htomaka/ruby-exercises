...
class SelectPatternTest < Minitest::Test
  ...
  def test_6
    ...
    words_with_c = {}
    furniture.each do |place, item|
      words_with_c[place] = item if item.include? "c"
    end
  ...
  def test_8
    ...
    two_words = {};
    meal.each do |course, meal|
      two_words[course] = meal if meal.split(' ').length == 2
    end
  ...
  def test_10
    ...
    items.each do |key, val|
      floats[key] = val if val.is_a? Float 
    end
